...
--data_path data/imagenet  --arch resnet50 --headinit none --exp_mode finetune \
--epochs 90 --batch_size 32 --lr 0.1 --wd 1e-4 \
--nesterov False --scheduler_type step \
--decay_epochs 30 60 90 --sync_bn False --wd_skip_bn False || scontrol requeue $SLURM_JOB_ID


# srun  --output=${EXPERIMENT_PATH}/%j.out --error=${EXPERIMENT_PATH}/%j.err \
# python supervised.py  --dump_path ${EXPERIMENT_PATH} \
# --data_path data/imagenet  --arch resnet152 --headinit none --exp_mode save_val_prob \
# --epochs 90 --batch_size 32 --lr 0.1 --wd 1e-4 \
